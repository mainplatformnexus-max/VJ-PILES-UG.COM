...
export default function Home() {
  ...
  const [playingVideo, setPlayingVideo] = useState<{url: string, title: string, contentId: string, contentType: string} | null>(null)
  const [showSubscriptionModal, setShowSubscriptionModal] = useState(false)
  const [showAuthModal, setShowAuthModal] = useState(false)
  const [returnUrl, setReturnUrl] = useState<string>("")
  const [isMobile, setIsMobile] = useState(false)
  const [searchQuery, setSearchQuery] = useState('') // Added searchQuery state to handle search in grid
  ...
  const handleSearch = (query: string) => { // Added handleSearch to set search query instead of navigating
        ...
        <main className="flex-1 pt-2 md:pt-12 overflow-hidden flex flex-col md:flex-row pb-16 md:pb-0">
          <div className="flex-1 overflow-y-auto scrollbar-hide p-1 md:p-3">
            ...
            <PopularGrid 
              ...
              searchQuery={activeFilter === 'search' ? searchQuery : ''} // Pass searchQuery to PopularGrid when search is active
              onPlayVideo={setPlayingVideo}
              onShowSubscription={() => setShowSubscriptionModal(true)}
              onRequireAuth={handleRequireAuth}
            />
