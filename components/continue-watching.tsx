...
export default function ContinueWatching({ onPlayVideo }: ContinueWatchingProps) {
  ...
  const handleWatch = async (item: ContinueWatchingItem) => {
    try {
      const contentRef = ref(database, `${item.contentType === 'series' ? 'series' : item.contentType === 'original' ? 'originals' : 'movies'}/${item.contentId}`)
      const snapshot = await get(contentRef)
      
      if (snapshot.exists()) {
        const content = snapshot.val()
        const videoUrl = item.contentType === 'series' && content.episodes?.[0]?.streamlink 
