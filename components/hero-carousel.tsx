...
export default function HeroCarousel({ playingVideo, onPlayVideo, onShowSubscription, onRequireAuth }: HeroCarouselProps) {
  ...
  const handleWatchNow = async () => {
    ...
    if (item?.contentId) {
      try {
        ...
        const contentRef = ref(database, `${type === 'series' ? 'series' : type === 'original' ? 'originals' : 'movies'}/${item.contentId}`)
        const snapshot = await get(contentRef)
        
        if (snapshot.exists()) {
          const content = snapshot.val()
          const videoUrl = type === 'series' && content.episodes?.[0]?.streamlink 
  ...
  return (
    <div ref={carouselRef} className="relative md:h-[280px] h-[120px] rounded-lg overflow-hidden group cursor-pointer" onClick={handlePosterClick}>
      {/* Background Image */}
      <div
      ...
      <div className="absolute left-3 md:left-6 bottom-3 md:bottom-6 max-w-xl z-10">
        <h1 className="md:text-3xl text-base font-bold text-white mb-1 md:mb-2">{currentItem.title}</h1>
        {currentItem.subtitle && (
          <p className="text-[9px] md:text-xs text-white/90 mb-2 md:mb-3 line-clamp-1 md:line-clamp-2">{currentItem.subtitle}</p>
        )}
        <div className="flex items-center gap-1 md:gap-2">
          <button
            ...
            className="px-2 md:px-4 py-0.5 md:py-1 bg-[#e50914] hover:bg-[#f6121d] rounded-lg text-white text-[9px] md:text-xs font-bold transition"
          >
            Watch Now
          </button>
        </div>
